...
import { Github, Globe, BarChart3 } from 'lucide-react';
...
export default function ProjectCard({ project, dict, index }: { project: Project; dict: Dict; index: number }) {
  ...
  const color = project.color || '#ec4899';
      ...
      <div className={`relative max-w-6xl mx-auto grid md:grid-cols-2 gap-8 md:gap-12 items-center ${isEven ? '' : 'md:[direction:rtl]'}`}>
        <div className="md:[direction:ltr]">
          ...
          >
            {project.thumbnail ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-5xl sm:text-6xl font-bold font-display" style={{ color }}>
        ...
        <div className="md:[direction:ltr]">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span
              className="text-[10px] sm:text-xs font-mono tracking-[0.2em] uppercase inline-flex items-center gap-1.5"
              style={{ color }}
            >
              <BarChart3 size={12} />
              {String(index + 1).padStart(2, '0')} · {project.isMobile ? 'MOBILE' : 'DATA'}
            </span>
            {project.featured && (
          ...
          <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 sm:mb-3 break-words font-display" style={{ color }}>
            {project.i18n.name}
          </h3>

          {project.i18n.tagline && (
            <p className="text-base sm:text-lg text-rose-100/80 mb-3 sm:mb-4">{project.i18n.tagline}</p>
          )}

          {project.i18n.description && (
            <p className="text-sm sm:text-base text-rose-100/60 leading-relaxed mb-5 sm:mb-6">{project.i18n.description}</p>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            {project.links?.web && (
              <a
                ...
                style={{ background: color, color: '#1a0a18' }}
              >
            ...
            {project.repos?.filter((r) => r.isPublic).map((repo) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 px-3.5 py-2 sm:px-4 sm:py-2 rounded-lg glass hover:bg-pink-500/10 active:bg-pink-500/15 transition text-xs sm:text-sm"
              >
                <Github size={16} />
